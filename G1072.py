...
def MOSfromR(Q):
    ...
    for i in range(len(Q)):
        if Q[i] > 0 and Q[i] < 100:
            MOS[i] = (
                MOS_MIN
                + (MOS_MAX - MOS_MIN) / 100 * Q[i]
                + Q[i] * (Q[i] - 60) * (100 - Q[i]) * 7.0e-6
            )
        elif Q[i] >= 100:
            MOS[i] = MOS_MAX
        else:
...
def VU(bitrate, framerate, coding_res, coeff):
    ...
    IVU = (
        coeff[0]
        + coeff[1] * np.log(bitperpixel * bitrate)
        + coeff[2] * np.log(scaleratio)
    )
...
def IQ_Frame(fr, FLR, coeff):
    ...
    I_IQ_frames = (
        coeff[0] + coeff[1] * fr ** 2 + coeff[2] * fr + coeff[3] * np.log(FLR + 1)
    )
...
def VQ(bitrate, NumPixelPerFrame, framerate, TSpacketLossV, TSburstinessV, coeff):
    ...
    QcodV = (
        coeff[0] * np.exp(coeff[1] * BitPerPixel)
        + coeff[2] * ContentComplexity
        + coeff[3]
    )
    TSburstinessV = 0
    I_codn = np.minimum(QcodV, 65.0)
    LossMagnitudeNP = (
        (coeff[9] - I_codn)
        * TSpacketLossV
        / (I_codn * (coeff[10] * TSburstinessV + coeff[11]) + TSpacketLossV)
    )
    LossMagnitudeE = coeff[12] * np.exp(coeff[13] * LossMagnitudeNP) - coeff[12]
    I_tras = coeff[7] * np.log(coeff[8] * LossMagnitudeE + 1.0)
...
def FrameLR(br, fr, delay, pl):
    ...
    if delay < 16:
        ...
        AVG_fps = fr * np.exp(
            -(0.08526 + 0.00073 * fr + 1.425e-04 * br * fr)
            * (0.09656 * delay - 1.5)
            * pl
        )
